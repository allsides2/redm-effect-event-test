RegisterCommand("test_all_effects", function(source, args, rawCommand)
    Citizen.CreateThread(function()
        for dictionary, effects in pairs(ptfx_assets_non_looped) do
            for _, effect in ipairs(effects) do
                local is_particle_effect_active = false
                local current_ptfx_dictionary = dictionary
                local current_ptfx_name = effect
                ...
                if Citizen.InvokeNative(0x65BB72F29138F5D6, GetHashKey(current_ptfx_dictionary)) then
                    ...
                    current_ptfx_handle_id =  Citizen.InvokeNative(0xE6CFE43937061143,current_ptfx_name,PlayerPedId(),ptfx_offcet_x,ptfx_offcet_y,ptfx_offcet_z,ptfx_rot_x,ptfx_rot_y,ptfx_rot_z,ptfx_scale,ptfx_axis_x,ptfx_axis_y,ptfx_axis_z)
                    is_particle_effect_active = true

                    TriggerEvent("chatMessage", "SYSTEM", {255, 0, 0}, "| DICTIONARY: " .. current_ptfx_dictionary .. " | EFFECT: " .. current_ptfx_name.." |")
                else
                    print("error:dicionay not load")
                end
